...
function hideTermsPopup() {
    ...
    document.cookie = 'acceptedMedjilTerms=true; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/';
}
...
function checkAcceptedTerms() {
    ...
    for (const cookie of cookies) {
        ...
        if (name === 'acceptedMedjilTerms' && value === 'true') {
            return true;
        }
    }
    return false;
...
// Entry point: Check if the user has already accepted the terms, and if not, show the popup
...
// Event listener for the "Accept" button in the popup
document.getElementById('acceptTCButton').addEventListener('click', hideTermsPopup);
