...
def create_record(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            return redirect('accounts:user_account')#redirect('blog:post_detail', pk= post.pk)
    else:
...
################################################################################
# Handle Data
def reading_data(raw_data):
    message = ''
    data = []
    raw_data = raw_data.read().decode('utf-8').splitlines()
    lines = csv.reader(raw_data)
    headers = next(lines)
    if len(headers) != 4:
        message = "Please ensure that the data has four columns - [Pin No, Reading, Number of Reading, Stdev]"
    elif isinstance(headers[0], int):
        message = "Please include a header for each column"
    else:
        data = [line for line in lines]
        message = "Successfully submitted your data for processing"
    return data, message
                ...
                if BarCodeRangeParam.objects.filter(site_id = site_id).exists():
                    ...
                    if range_param[1]:
                        ...
                        if missingPins or newPins:
                            ...
                            #     print("Well Done")
                            messages.warning(request, 'Congratulations! You have successfully calibrated you staff.') 
                    ...
                    messages.warning(request, 'The calibration site - ' + site_id + ' does not exist. Please contact the site operator to add it.' )    
                    return redirect('staffcalibration:calibrate')
            else:
...
@login_required(login_url="/accounts/login")
def print_report(request, job_number):
    resp = HttpResponse(content_type='application/pdf')
    
    # Calibration ID
    thisRecord = StaffCalibrationRecord.objects.get(job_number=job_number)
