...
class Calibrated_BaselineInline(admin.TabularInline):
    model = Calibrated_Baseline
    extra = 0


@admin.register(Calibrated_Baseline)
class Calibrated_BaselineAdmin(admin.ModelAdmin):
    #list_display = ('pillar_survey')
    #list_filter = ('pillar_survey__baseline',)
    fields = ['pillar_survey',
               ('zero_point_correction','zpc_uncertainty'),
               'degrees_of_freedom']
    inlines = [Certified_DistanceInline, 
    					 Std_Deviation_MatrixInline]
...
class Pillar_SurveyAdmin(admin.ModelAdmin):
    ...
    inlines = [EDM_ObservationInline, Level_ObservationInline, Calibrated_BaselineInline]
...
class EDM_ObservationAdmin(admin.ModelAdmin):
    list_display = ('baseline_pillar', 'pillar_survey')
