...
from math import sqrt, exp, sin, radians
...
def adjust_alignment_survey(raw_edm_obs, pillars):
    ...
    for p in setups.values():
        ...
        d0, rot0 = joins(setups[first_pillar_nme]['Est'],setups[first_pillar_nme]['Nth'],
                     setups[last_pillar_nme]['Est'],setups[last_pillar_nme]['Nth'])
        # find the arbitary azimuth from first to last pillar
        ...
        # join the differnece between the 2 Azimuths to calc Offset
...
def offset_slope_correction(o, level_observations, alignment_survey, d_radius):
    avg_baseline_h = mean([float(o['reduced_level']) for o in level_observations.values()])
    ...
    d_height_factor = ((1+(rl1-avg_baseline_h)
                       /(d_radius+avg_baseline_h))
                       * (1+(rl2-avg_baseline_h)
                          /(d_radius+avg_baseline_h)))
...
def refline_std_dev(o, alignment_survey, edm):
    ...
    # '07' uncertianty distance as constant
    if '07' in uc_budget.keys(): 
        uc_budget['07']['ui'] = uc_budget['07']['std_dev']
        
    # '08' uncertianty of distance as ratio
    if '08' in uc_budget.keys():
        uc_budget['08']['ui'] = o['Reduced_distance'] * uc_budget['08']['std_dev']
...
def add_surveyed_uc(o, edm_trend, uc_sources, alignment_survey):
    ...
    surveyed_uc.append({'group': '02',
                   ...
                   'description':'from trendline to EDM distance standard deviations'})
    ...
    surveyed_uc.append({'group': '10',
                        ...
                        'description':'Surveyed pillar height'})
       
    # '11'
    frm_os=alignment_survey[o['from_pillar']]
    to_os=alignment_survey[o['to_pillar']]
    if 'OS_std_dev' not in frm_os:
        frm_os['OS_std_dev']=float(frm_os['os_uncertainty'])/float(frm_os['k_os_uncertainty'])
    if 'OS_std_dev' not in to_os:
        to_os['OS_std_dev']=float(to_os['os_uncertainty'])/float(to_os['k_os_uncertainty'])   
...
def add_certified_dist_uc(o, uc_sources, std_dev_matrix, dof):
    ...
    cd_uc.append({'group': '07',
                  ...
                  'description':'Uncertainty of Certified Distances'})
...
def add_calib_uc(uc_sources, calib, insts):
    ...
    if calib['them']:
        uc_sources.append({'group': '04',
                           ...
                           'description':'Themometer calibrated correction factor'})
        ...
        uc_sources.append({'group': '04',
                           ...
                           'k':sqrt(3),
                           'description':'Themometer Rounding'})
